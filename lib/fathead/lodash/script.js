...
function getDetails($elem) {
    ...
    `<pre>${abstractData.title}</pre>` +
    `<p>${abstractData.summary}</p>` +
    `<span class="prog__sub">Arguments</span>${abstractData.args}` +
    `<span class="prog__sub">Returns</span><pre>${abstractData.returns}</pre>` +
    `<span class="prog__sub">Example</span>${abstractData.example}</section>`.replace(/\t/g,  '    ').replace(/\n/g, "\\n");
