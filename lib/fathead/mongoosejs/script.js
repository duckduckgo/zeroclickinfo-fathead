...
(function organize() {
	...
	let disambiguations = getDisambiguations(outputData);
...
function getCategory(article) {
	if(article.indexOf("-") !== -1) {
		return article.slice(0, article.lastIndexOf("-"));
...
function getDisambiguations(docs) {
	...
	docs.map(({article}) => {
		let findThis = article.slice(article.lastIndexOf("-") + 1, article.length); //Extract article (Mongoose-Aggeragate -> Aggeragate)
		let disambiguations = findDisambiguations(docs, findThis, alreadyWritten);
		if(disambiguations.queries.indexOf("*") !== -1) similar.push(disambiguations); //For empty disambiguations
