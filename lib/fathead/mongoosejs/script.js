...
function findDisambiguations(docs) {
	let similar = [];
	docs.map(current => {
		let findThis = current.article.slice(current.article.lastIndexOf("-") + 1, current.article.length);
		let tempSimilar = {userQuery: findThis, queries: "*"};
		docs.map(({article, disambiguation}) => {
			let reg = new RegExp("^" + findThis + "$", "g");
			let slicedArt = article.slice(article.lastIndexOf("-") + 1, article.length);
			if(reg.test(slicedArt) && current.article !== article) tempSimilar.queries += `[[${article}]], ${disambiguation}\\n*`;
		});
		tempSimilar.queries = tempSimilar.queries.slice(0, tempSimilar.queries.length - 3);
