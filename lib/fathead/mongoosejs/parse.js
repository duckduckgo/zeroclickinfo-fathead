...
function getDisambiguations(docs) {
	...
	docs.map(function (current) {
		var findThis = current.article.slice(current.article.lastIndexOf("-") + 1, current.article.length); //Extract article (Mongoose-Aggeragate -> Aggeragate)
		var tempSimilar = findDisambiguations(docs, findThis, alreadyWritten);
		if ((tempSimilar.queries.match(/\\n*/g) || []).length) similar.push(tempSimilar);
		alreadyWritten.push(findThis); //for duplicates
	});
	return similar.map(function (_ref) {
		var userQuery = _ref.userQuery,
...
function findDisambiguations(docs, findThis, alreadyWritten) {
	...
	docs.map(function (_ref2) {
		var article = _ref2.article,
		    disambiguation = _ref2.disambiguation;

		var slicedArt = void 0;
		var reg = new RegExp("^" + findThis + "$", "g");
		article.lastIndexOf("-") === -1 ? slicedArt = article.slice(article.lastIndexOf(".") + 1, article.length) : slicedArt = article.slice(article.lastIndexOf("-") + 1, article.length);
...
function getDetails($elem) {
	...
	if (article.indexOf("-") === -1 && article.indexOf(".") === -1) article = "-" + article;
