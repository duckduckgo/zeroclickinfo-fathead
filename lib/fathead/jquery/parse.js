...
if (require.main === module) {
  ...
  Promise.all(process.argv.slice(2).map(path => {
    return readXMLDoc(path).then(doc => {
      ...
      const title = xpath('//entry/title', doc)[0].textContent
    ...
    util.groupBy('title', pages.filter(page => {
      // Only create disambiguation pages for redirects which don’t have the
      // same name as an existing article.
      return page.type === 'R' && !articles.has(page.title)
    })).forEach((redirects, title) => {
      const disambiguations = Array.from(redirects.reduce((set, redirect) => {
        const target = redirect.redirectTitle
        const summary = articles.get(target).abstract.match(/[^.]*/)[0]
        return set.add(`*[[${target}]] ${summary}.`)
      }, new Set())).sort()

      articles.set(title, disambiguations.length === 1 ? redirects[0] : {
        title: title,
        type: 'D', // disambiguation
        disambiguationContent: disambiguations.join('\n')
      })
    })
