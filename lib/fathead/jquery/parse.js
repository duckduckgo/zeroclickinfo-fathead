...
function removeTrailingParens (title) {
  return title
    .replace(/\(\)$/, '')
}

function removeLeadingDot (title) {
  return title
    .replace(/^\./, '')
}
...
if (require.main === module) {
  ...
  paths.forEach(path => {
    readFile(path, (err, data) => {
      ...
      const redirects = new Set([title, removeContext(title)])

      Array.from(redirects).forEach(redirect => {
        redirects.add(removeLeadingDot(redirect))
      })

      Array.from(redirects).forEach(redirect => {
        redirects.add(removeTrailingParens(redirect))
      })

      Array.from(redirects).forEach(redirect => {
        redirects.add(removeSpecialCharacters(redirect))
        redirects.add(removeSpecialCharacters(splitSubwords(redirect)))
      })
