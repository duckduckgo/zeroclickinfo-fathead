...
function makeOutput(docs) {
	let logo = "http://docs.celeryproject.org/en/stable/_static/celery_512.png";
...
function formatSummary($elem) {
	...
	for(let i = 0; i < $elem.length; i+=1) {
		let $tr = $elem.eq(i);
		let name = `<span class="prog__sub">${$tr.children("th").text().replace(/:/g, "")}</span>`;
		let body = $tr.find("ul") ? summaryHelper($tr) : $tr.children("td").text();
		summary.push(`${name}\n${body}`);
	}
	return summary.join("");
}
