...
function summaryHelper($tr) {
	let $ul = $tr.children("td").find("ul");
	if(!/<\/ul>/.test($tr.html())) return $tr.children("td").text() ? `<p>${$tr.children("td").text()}</p>` : "";
...
function formatSummary($elem) {
	...
	for(let i = 0; i < $elem.length; i+=1) {
		let $tr = $elem.eq(i);
		let name = !$tr.children("th").text() ? "" : `<span class="prog__sub">${$tr.children("th").text().replace(/:/g, "")}</span>`;
