...
function makeOutput(docs) {
	var logo = "http://docs.celeryproject.org/en/stable/_static/celery_512.png";
...
function summaryHelper($tr) {
	var $ul = $tr.find("ul");
	if (!$ul.html()) return "<p>" + $tr.children("td").text() + "</p>";
...
function formatSummary($elem) {
	...
	for (var i = 0; i < $elem.length; i += 1) {
		var $tr = $elem.eq(i);
		var name = "<span class=\"prog__sub\">" + $tr.children("th").text().replace(/:/g, "") + "</span>";
		var body = $tr.find("ul") ? summaryHelper($tr) : $tr.children("td").text();
		summary.push(name + "\n" + body);
	}
	return summary.join("");
}
...
function makeAbstract($elem) {
	var abstractData = {
		...
		example: ("<pre><code>" + $elem.find(".highlight pre").text() + "</code></pre>").split("...").join("\n"),
