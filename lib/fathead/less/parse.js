...
function makeOutput(docs) {
	var logo = "http://lesscss.org/public/img/logo.png";
...
function getRedirects(article) {
	return ["css", "lesscss", "less css"].map(function (newArticle) {
		return newArticle + " " + article + "\tR\t" + article + "\t\t\t\t\t\t\t\t\t\t\n";
	});
}
...
function getDocs(lessData) {
	...
	for (var i = 1; i < $docs.length; i += 1) {
		...
		var category = ("LESS " + $docs.eq(i).find(".page-header h1").text()).replace("\n", "");
