...
function formatDom(desc) {
	if (!desc) return;
	return desc.replace(/<\s*a[^>]*>/ig, "").replace(/<\s*\/\s*a>/ig, "").replace(/<\s*img[^>]*>/gi, "").replace(/<*\/*img>/ig, "");
}
...
function makeAbstract(doc) {
	...
	for (var i = 5; i < doc.length; i += 1) {
		if (/<br>|^\s+$/g.test(doc[i])) continue;
		if (/:/g.test(doc[i]) && /<\/p>/g.test(doc[i])) {
			doc[i] = doc[i].replace("<p>", "<span class=\"prog__sub\">").replace(":", "</span>").replace("</p>", "");
		}
