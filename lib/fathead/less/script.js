...
function makeOutput(docs) {
	let logo = `http://lesscss.org/public/img/logo.png`;
...
function getRedirects(article) {
	return ["css", "lesscss", "less css"]
		.map(newArticle => `${newArticle} ${article}\tR\t${article}\t\t\t\t\t\t\t\t\t\t\n`);
}
...
function getDocs(lessData) {
	...
	for(let i = 1; i < $docs.length; i+=1) {
		...
		let category = `LESS ${$docs.eq(i).find(".page-header h1").text()}`.replace("\n", "");
