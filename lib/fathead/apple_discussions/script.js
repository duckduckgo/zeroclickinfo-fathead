...
async function getDocs(sitemap) {
	try {
		...
		return threads.filter(thread => { //Checks if the thread has an accepted Answer
...
function getDetails({url, docHtml}) {
	...
	let linkToAnswer = `#${$("[itemprop=acceptedAnswer]").attr("id")}`;
...
function formatDom(dom) {
	...
	return dom.replace(/<\s*a[^>]*>/ig, "")
		...
		.replace(/<\s*img[^>]*>/ig, "") //Remove images
